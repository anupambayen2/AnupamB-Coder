...
def apply_stage_config(cfg: TrainConfig, stage):
    """Apply LR and step config for each stage."""
    ...
    # ── Stage 3 sub-stages ────────────────────────────────────
    # Lower LR each sub-stage to refine without overwriting
    elif stage == "3a":
...
def get_dataloaders(stage, batch_size: int,
                    block_size: int):
    ...
    if stage in stage3_stages:
        from src.data.dataset_stage3 import (
            build_dataloaders_stage3
        )
...
class Logger:
    ...
    def print_step(self, step, loss, lr, dt_ms,
                   vram_gb=None, max_steps=None):
        vram_s = (
            f"  vram {vram_gb:.1f}GB" if vram_gb else ""
        )
        time_s = ""
        if max_steps:
...
def train(resume_from=None, stage=1, max_steps=None):
    ...
    while step < cfg.max_steps:
        ...
        for _ in range(cfg.grad_accum_steps):
            ...
            x, y = x.to(cfg.device), y.to(cfg.device)
            with autocast(
                device_type="cuda", enabled=cfg.fp16
            ):
                _, loss = model(x, y)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type    = str,
        default = "1",
        help    = "Stage: 1, 2, 3a, 3b, 3c, 3d, 4, 5, 6",
    )
    parser.add_argument(
        "--max_steps",
        type    = int,
